...
export default class Trx {
    ...
    async sign(
        ...
        let { functionSelector, ...transaction } = transactionData;
        ...
        try {
            ...
            /**
             * When the transaction is generated by TriggerSmartContract,
             * get the function_selector and pass it to the native app
             */
            const [contract] = transaction.raw_data.contract;
            if (!functionSelector && contract.type === 'TriggerSmartContract') {
                const intIds = window.transitionPostMessage.idMapping.intIds;
                const parameterData = contract.parameter.value.data.substr(8);
                const functionSelectorData = intIds.get(parameterData);
                if (functionSelectorData) {
                    functionSelector = functionSelectorData;
                    intIds.delete(parameterData);
                }
            }
